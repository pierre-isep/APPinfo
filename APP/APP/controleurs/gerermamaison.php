...
switch ($function) {
    ...
    case 'ajout_cemac':
        ...
        if (isset($_POST['pièce']) and isset($_POST['numero']))
        {
            ...
            $req = $bdd->prepare('SELECT ID_CeMAC FROM cemac WHERE ID_piece = :ID_piece AND numero = :numero  ');
            $req->execute(array(
                'ID_piece' => $_POST['pièce'],
                'numero' => $_POST['numero']));
            $nouvelle_id_cemac = $req->fetch();

            $req = $bdd->prepare('INSERT INTO actionneur( ID_CeMAC) VALUES( :ID_CeMAC)');

            $req->execute(array(
                'ID_CeMAC' => $nouvelle_id_cemac['ID_CeMAC']));
        }
